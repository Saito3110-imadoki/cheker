...
def generate_posts_with_claude(
    ...
    prompt = (
        ...
        "複数枚にする場合は必ず「起承転結」のストーリーで組み立てること:\n"
        "- 起（1枚目・表紙）: 問題提起・共感フック。「あ、自分のことだ」と思わせて手を止めさせる\n"
        "- 承（2枚目）: 背景・原因の深掘り。なぜその問題が起きるのかを納得させる\n"
        "- 転（3枚目）: 視点の転換・解決策。「実は◯◯だった」という一番の見せ場\n"
        "- 結（最終枚）: まとめと次の一歩。読者が今日やることを1つ提示して保存を促す\n"
        "- 3枚の場合は 起→転→結、2枚の場合は 起→結 に圧縮する\n"
        "- 各チャートに \"role\" フィールド（起/承/転/結 のいずれか1文字）を必ず入れること（画像右上に表示される）\n"
