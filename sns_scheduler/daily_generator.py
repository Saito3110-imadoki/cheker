...
def generate_posts_with_claude(
    ...
    prompt = (
        ...
        "   text は40文字以内で具体的に。steps は2〜4個\n"
        "   impact には読者への行動提案を1文（30文字以内・任意）\n"
        '   {"chart_type":"flow","title":"結論型タイトル","subtitle":"補足（任意）",'
        '"steps":[{"label":"原因","text":"SNS投稿の反応が悪いのは内容ではなく1行目の問題"},'
        '{"label":"仮説","text":"タイムラインでは1行目しか読まれていない"},'
        '{"label":"対策","text":"数字・損失回避・意外性のいずれかで書き出す"},'
        '{"label":"次の一歩","text":"過去投稿の1行目だけを書き直してみる"}],'
        '"impact":"1行目を変えるだけで反応は2倍変わる","caption":"出典（任意）"}\n\n'
        "⑤ list（ポイント解説）: コツ・チェックリスト・要点まとめ\n"
        "   head は12文字以内の見出し、text は35文字以内の補足。items は3〜4個\n"
        '   {"chart_type":"list","title":"結論型タイトル","subtitle":"補足（任意）",'
        '"items":[{"head":"完璧を目指さない","text":"6割の出来で投稿して反応から学ぶ方が速い"},'
        '{"head":"数字を1つ入れる","text":"具体的な数字は信頼と保存率を上げる"},'
        '{"head":"問いかけで締める","text":"リプライはアルゴリズム評価が最も高い"}],'
